...
vlesslink1="vless://${uuid}@${domain}:443?path=/YOURPATH/vless&security=tls&encryption=none&host=${domain}&type=ws&sni=${domain}#${user}"
vlesslink2="vless://${uuid}@${domain}:80?path=/YOURPATH/vless&security=none&encryption=none&host=${domain}&type=ws#${user}"
vlesslink3="vless://${uuid}@${domain}:443?mode=gun&security=tls&encryption=none&type=grpc&serviceName=vless-grpc&sni=${domain}#${user}"
vless1="vless://${uuid}@${domain}:443?path=/YOURPATH/vless%26security=tls%26encryption=none%26host=${domain}%26type=ws%26sni=${domain}#${user}"
vless2="vless://${uuid}@${domain}:80?path=/YOURPATH/vless%26security=none%26encryption=none%26host=${domain}%26type=ws#${user}"
...
Link Vless Account
_______________________________
Link TLS : vless://${uuid}@${domain}:443?path=/YOURPATH/vless&security=tls&encryption=none&host=${domain}&type=ws&sni=${domain}#${user}
_______________________________
Link none TLS : vless://${uuid}@${domain}:80?path=/YOURPATH/vless&security=none&encryption=none&host=${domain}&type=ws#${user}
_______________________________
...
echo -e "$COLOR1 ${NC} ${WH}Path         ${COLOR1}: ${WH}/YOURPATH/vless" | tee -a /etc/vless/akun/log-create-${user}.log
...
expiration_time=$((creation_time + timer * 60))
# Mengonversi waktu pembuatan dan waktu expired ke format yang lebih mudah dibaca
creation_time_human=$(date -d @$creation_time +"%H:%M:%S")
expiration_time_human=$(date -d @$expiration_time +"%H:%M:%S")
exp=`date -d "$masaaktif days" +"%Y-%m-%d"`
sed -i '/#vless$/a\#vl '"$user $exp $uuid"'\
},{"id": "'""$uuid""'","email": "'""$user""'"' /etc/xray/config.json
sed -i '/#vlessgrpc$/a\#vlg '"$user $exp"'\
...
echo "rm -f /etc/vmess/${user} /etc/vmess/${user}IP && sed -i '/$user/d' /etc/xray/config.json && systemctl restart xray" | at now + $timer minutes
cat > /home/vps/public_html/vless-$user.txt <<-END
_______________________________
Format Vless WS (CDN)
_______________________________
...
Link TLS : vless://${uuid}@${domain}:443?path=/YOURPATH/vless&security=tls&encryption=none&host=${domain}&type=ws&sni=${domain}#${user}
_______________________________
Link NTLS : vless://${uuid}@${domain}:80?path=/YOURPATH/vless&security=none&encryption=none&host=${domain}&type=ws#${user}
_______________________________
Link gRPC : vless://${uuid}@${domain}:443?mode=gun&security=tls&encryption=none&type=grpc&serviceName=vless-grpc&sni=${domain}#${user}
_______________________________
END
...
echo -e "$COLOR1 $NC ${WH}Waktu Dibuat ${COLOR1}: ${WH}$creation_time_human"  | tee -a /etc/vmess/akun/log-create-${user}.log
echo -e "$COLOR1 $NC ${WH}Waktu Berakhir ${COLOR1}: ${WH}$expiration_time_human"  | tee -a /etc/vmess/akun/log-create-${user}.log
echo -e "$COLOR1 ◇━━━━━━━━━━━━━━━━━◇ ${NC}" | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1 ${NC} ${WH}    $author    " | tee -a /etc/vless/akun/log-create-${user}.log
echo -e "$COLOR1 ◇━━━━━━━━━━━━━━━━━◇ ${NC}" | tee -a /etc/vless/akun/log-create-${user}.log
echo "" | tee -a /etc/vless/akun/log-create-${user}.log
